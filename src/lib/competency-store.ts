...
export async function getCompetencies(): Promise<Competency[]> {
  if (await isDbAvailable()) {
    const data = await pgGetCollection<Competency[]>('competencies');
    if (data && data.length > 0) return data;
    await pgSetCollection('competencies', DEFAULT_COMPETENCIES);
...
export async function updateCompetencies(competencies: Competency[]): Promise<Competency[]> {
  if (await isDbAvailable()) {
    await pgSetCollection('competencies', competencies);
  } else {
    await ensureDir();
